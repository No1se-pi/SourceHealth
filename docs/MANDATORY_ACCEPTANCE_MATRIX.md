...
| **M-15** | **SourceCraft API + CLI / документация**<br>(Интеграция с платформой и действующие интерфейсы) | Server-side интеграция с SourceCraft реализована напрямую через публичный [REST API](https://sourcecraft.dev/portal/docs/ru/sourcecraft/operations/api-start) (`https://api.sourcecraft.tech`), официальный AppSec API и Git-over-HTTPS/PAT. Официальный SourceCraft CLI [`src`](https://sourcecraft.dev/portal/docs/ru/cli-ref/src) существует и функционально шире логина (включает `src api`, `src clone`, `src appsec`, `src repo`, `src run`), однако отдельный бинарник `src` в текущей server-side реализации не вызывается, так как REST API документирован платформой именно как прямой интерфейс для автоматизации и интеграций. Собственный CLI SourceHealth (`probe-sourcecraft`, `discover`, `worker`) обеспечивает автоматизацию и валидацию | `tests/test_appsec.py`, `tests/test_foundation.py`, `tests/test_acceptance.py` (CLI probe tests), `docs/SOURCECRAFT.md` | Справочная документация в `docs/SOURCECRAFT.md` со ссылками на официальный Swagger, REST API и CLI ([src](https://sourcecraft.dev/portal/docs/ru/cli-ref/src), [src api](https://sourcecraft.dev/portal/docs/en/cli-ref/src-api)) | **PASS / EVIDENCED INTERPRETATION** | Публичный REST API и Git-over-HTTPS/PAT являются официально документированными интерфейсами интеграции платформы; отдельный `src` executable на сервере не вызывается. |
| **M-16** | **Воспроизводимый билд и деплой**<br>(Docker Compose, Caddy, Systemd) | `deploy/compose.prod.yaml`, `deploy/Caddyfile`, systemd service/timer units. Устранены PR #15 blockers. Caddy syntax: `caddy validate --config deploy/Caddyfile` (exit code 0, Valid configuration). Caddy behavioral routing: `/api/v1/health` $\to$ 200 backend JSON, `/some/spa/path` $\to$ 200 SPA index.html, `/api/not-real` $\to$ 404 backend JSON (не index.html) | `docker compose config --quiet` (PASS), `docker compose -f deploy/compose.prod.yaml config --quiet` (PASS), `caddy validate` (exit code 0), behavioral routing test (PASS) | Готовый deploy bundle с mutual-exclusive routing `/api/*` и SPA fallback | **PASS** | Устранены плейсхолдерные overrides в systemd units; задокументированы URL-safe пароли. |
...
- Веса категорий: Documentation (15), CI/CD (15), Security (20), Activity (15), Issues (15), Code Health (20).
...
- Server-side интеграция с SourceCraft реализована через публичный [REST API](https://sourcecraft.dev/portal/docs/ru/sourcecraft/operations/api-start) (`api.sourcecraft.tech`), официальный AppSec API и стандартный Git-over-HTTPS/PAT (`git clone`, `git ls-files`).
- Официальный SourceCraft CLI [`src`](https://sourcecraft.dev/portal/docs/ru/cli-ref/src) существует и предоставляет полноценный набор команд (`src api`, `src clone`, `src appsec`, `src repo`, `src run`), включая [интерфейс работы с API](https://sourcecraft.dev/portal/docs/en/cli-ref/src-api). Однако отдельный исполняемый файл `src` в текущей server-side реализации SourceHealth не вызывается, поскольку публичный REST API документирован платформой как прямой интерфейс автоматизации и интеграции.
- Собственный CLI SourceHealth (`probe-sourcecraft`, `discover`, `worker`) полностью реализован и протестирован (`test_acceptance.py`).
- Документация в `docs/SOURCECRAFT.md`.
- Статус: **PASS / EVIDENCED INTERPRETATION**.
...
- Валидация Caddy:
  - Синтаксис: `caddy validate --config deploy/Caddyfile` — EXIT CODE 0 (`Valid configuration`).
  - Behavioral routing test: `/api/v1/health` $\to$ 200 backend JSON, `/some/spa/path` $\to$ 200 SPA `index.html`, `/api/not-real` $\to$ 404 backend JSON (не SPA index.html) — все проверки пройдены (EXIT CODE 0).
- Статус: **PASS**.
