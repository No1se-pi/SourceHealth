...
| **M-15** | **SourceCraft API + CLI / документация**<br>(Интеграция с платформой и действующие интерфейсы) | Программная интеграция с SourceCraft реализована через официальный REST API (`https://api.sourcecraft.tech`), официальный AppSec API и Git CLI (`git`). Официальный бинарник `src` (SourceCraft CLI) не вызывается кодом, так как предназначен для интерактивного терминального логина разработчика (`src auth login`), а серверная интеграция документирована через REST API и Git over HTTPS/PAT. Собственный CLI SourceHealth (`probe-sourcecraft`, `discover`, `worker`) обеспечивает автоматизацию и валидацию | `tests/test_appsec.py`, `tests/test_foundation.py`, `tests/test_acceptance.py` (CLI probe tests), `docs/SOURCECRAFT.md` | Справочная документация в `docs/SOURCECRAFT.md` со ссылками на официальный Swagger и REST API | **PASS / EVIDENCED INTERPRETATION** | Официальный REST API и Git endpoint являются документированными программными интерфейсами платформы; отдельный `src` executable для серверного анализа не требуется. |
...
- Программная интеграция: официальный REST API (`api.sourcecraft.tech`), официальный AppSec API и Git CLI (`git clone`, `git ls-files`).
- Официальный бинарник SourceCraft CLI (`src auth login`) предназначен для терминального интерактивного логина разработчика; для серверного бэкенда документированным внешним интерфейсом интеграции выступает REST API.
