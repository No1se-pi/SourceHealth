...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(error.code ?? 'request_failed', response.status, error.request_id ?? '');
  }
  return response.json() as Promise<T>;
}

export const api = {
  repositories: (offset = 0, sort = 'health_score', language = '') => request<RepositoryPage>(
    `/repositories?${new URLSearchParams({ offset: String(offset), sort, ...(language ? { language } : {}) })}`),
  repository: (id: string) => request<RepositoryDetails>(`/repositories/${encodeURIComponent(id)}`),
  analysis: (id: string) => request<Analysis>(`/analyses/${encodeURIComponent(id)}`),
  start: (id: string) => request<AnalysisSummary>(`/repositories/${encodeURIComponent(id)}/analyses`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ force_refresh: false }),
  }),
  me: () => request<components['schemas']['UserDTO']>('/me'),
};
