...
def unavailable(name: str, code: str) -> AnalyzerResult:
    category, source = CLASSIFICATION[name]
...
def normalize_runtime_report(payload: dict) -> dict[str, AnalyzerResult]:
    ...
    if (not isinstance(payload, dict) or payload.get("schema_version") != "1.0"
        ...
        return {name: unavailable(name, "runtime_invalid_report") for name in CLASSIFICATION}
    error = payload.get("error")
    code = error.get("code") if isinstance(error, dict) else None
    code = code if isinstance(code, str) and code in RUNTIME_ERRORS else "runtime_failed"
    results = {}
    for name, converter in (("git_activity", git_result), ("sast", sast_result)):
